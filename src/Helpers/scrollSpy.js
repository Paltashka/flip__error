...
export default class ScrollSpy extends React.PureComponent {
  ...
  render () {
    ...
    let counter = 0;
    let inViewState = this.state.inViewState||[];
    let new_items = React.Children
        .map(children, (child, idx) => !child?null:({child:child.props.children.props.children, idx,inViewState:inViewState[idx]||false}))
        .filter(f=>f!==null&&f.inViewState===true).reduce((a,c)=>c,{});
    ...
    return (
      <>
        { new_items.child||<div>Catalog</div> }
      </>
    )
  }
}
